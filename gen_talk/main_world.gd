...
func _ready() -> void:
	...
	randomize()
	for i in range(total_npcs_to_spawn):
		_generate_and_place_npc()

func _generate_and_place_npc() -> void:
	if npc_scene == null: return
	var new_npc_instance = npc_scene.instantiate()
	
	var random_x = randf_range(spawn_boundary_min.x, spawn_boundary_max.x)
	var random_y = randf_range(spawn_boundary_min.y, spawn_boundary_max.y)
	
	new_npc_instance.position = Vector2(random_x, random_y)
